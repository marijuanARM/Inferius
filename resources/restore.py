...
def send_bootchain(processor, is_verbose):
    if processor.lower() == 's5l8960' or processor.lower() == 't8015':
        subprocess.run(('./resources/bin/irecovery', '-f', 'work/ipsw/ibss.img4'), stdout=subprocess.PIPE) # A7/A11 need to reset their usb connection, this does the job

    ibss_send = subprocess.run(('./resources/bin/irecovery', '-f', 'work/ipsw/ibss.img4'), stdout=subprocess.PIPE, universal_newlines=True)
    utils.log(ibss_send.stdout, False)
    utils.log('[VERBOSE] Sent iBSS', is_verbose)

    ibec_send = subprocess.run(('./resources/bin/irecovery', '-f', 'work/ipsw/ibec.img4'), stdout=subprocess.PIPE, universal_newlines=True)
...
def save_blobs(device_identifier, firm_bundle_number, board_configs, downgrade_10, is_verbose):
    ...
    if board_configs:
        fetch_blobs = subprocess.run(('./resources/bin/tsschecker', '-d', device_identifier, blob_version, '-B', f'{board_configs[firm_bundle_number]}ap', '-e', f'0x{ecid}', '-s', '--apnonce', apnonce, '--save-path', 'work/ipsw'), stdout=subprocess.PIPE, universal_newlines=True)
    else:
        fetch_blobs = subprocess.run(('./resources/bin/tsschecker', '-d', device_identifier, blob_version, '-e', f'0x{ecid}', '-s', '--apnonce', apnonce, '--save-path', 'work/ipsw'), stdout=subprocess.PIPE, universal_newlines=True)
    ...
    if ipsw.is_a7(device_identifier):
        fetch_blobs = subprocess.run(('./resources/bin/tsschecker', '-d', device_identifier, blob_version, '-e', f'0x{ecid}', '-s', '--save-path', 'work/ipsw'), stdout=subprocess.PIPE, universal_newlines=True)
        utils.log(fetch_blobs.stdout, False)
...
def sign_bootchain(ibss_path, ibec_path, is_verbose):
    ...
    sign_ibss = subprocess.run(('./resources/bin/img4tool', '-c', 'work/ipsw/ibss.img4', '-p', ibss_path, '-s', shsh_path), stdout=subprocess.PIPE, universal_newlines=True)
    ...
    sign_ibec = subprocess.run(('./resources/bin/img4tool', '-c', 'work/ipsw/ibec.img4', '-p', ibec_path, '-s', shsh_path), stdout=subprocess.PIPE, universal_newlines=True)
